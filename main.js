...
function updateAlertBadge() {
  ...
  if (currentUser && (currentUser.role === 'sales' || currentUser.role === 'sales_counter')) {
    ...
    // Technical support: klaim yang belum diambil ATAU diambil saya tapi belum selesai diproses
    count = typeof TECH_NOTIFICATIONS !== 'undefined'
      ? TECH_NOTIFICATIONS.filter(n => {
          const claim = CLAIMS.find(c => c.id === n.claimId);
          const isDone = claim && claim.processDone;
          return !isDone && (!n.takenBy || n.takenBy === currentUser.id);
        }).length
      : 0;
  } else {
...
function renderAlertsTechSupport() {
  // Sembunyikan notifikasi yang sudah selesai diproses (punya foto & done)
  const myNotifs = TECH_NOTIFICATIONS.filter(n => {
    const claim = CLAIMS.find(c => c.id === n.claimId);
    return !(claim && claim.processPhotos && claim.processPhotos.length > 0 && claim.processDone);
  });
