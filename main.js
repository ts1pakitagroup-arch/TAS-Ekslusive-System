...
function updateAlertBadge() {
  ...
  if (currentUser && (currentUser.role === 'sales' || currentUser.role === 'sales_counter')) {
    ...
    // Technical support: klaim yang belum diambil ATAU klaim yang ditolak (perlu upload ulang)
    count = typeof TECH_NOTIFICATIONS !== 'undefined'
      ? TECH_NOTIFICATIONS.filter(n => {
          if (n.takenBy && n.takenBy !== currentUser.id) return false;
          const claim = CLAIMS.find(c => c.id === n.claimId);
          if (!claim) return true;
          if (claim.status === 'Approved') return false; // hilang jika approved
          if (claim.status === 'Rejected') return true;  // tetap jika rejected
          return !n.takenBy; // belum diambil siapapun
        }).length
      : 0;
  } else {
...
function filteredVehicles() {
  // Combine both monitoring and trial vehicles
  const allVehicles = [...vehicles, ...trialVehicles];
  
  // Filter by current user
  let userFiltered = allVehicles;
  if (currentUser) {
    // Administrator can see all vehicles
    if (currentUser.role !== 'administrator') {
      userFiltered = allVehicles.filter(v => v.createdBy === currentUser.id);
    }
  }
  
  // Apply search filter
  if (!searchQuery) return userFiltered;
  return userFiltered.filter(v => 
...
function renderMonitoring() {
  ...
  // Filter by current user - only show vehicles created by this user
  const filterByUser = (vehicleList) => {
    if (!currentUser) return [];
    // Administrator & Supervisor can see all data
    if (currentUser.role === 'administrator' || currentUser.role === 'supervisor') {
      return vehicleList;
    }
    // Other users only see their own data
    return vehicleList.filter(v => v.createdBy === currentUser.id);
  };
  
  const activeList = filterByUser(isMonTab ? vehicles : trialVehicles);
  ...
  const filteredVehiclesCount = filterByUser(vehicles).length + filterByUser(trialVehicles).length;
  const filteredPelumasCount = filterPelumasByUser(pelumasRecords).length + filterPelumasByUser(pelumasTrialRecords).length;
  ...
  <div style="display:flex;gap:12px;margin-bottom:28px;flex-wrap:wrap;">
    ...
    <button onclick="monitoringCategory='${key}';render()" style="display:inline-flex;align-items:center;gap:10px;padding:14px 24px;border-radius:18px;border:2px solid ${monitoringCategory===key ? c.color : 'var(--border)'};background:${monitoringCategory===key ? c.bg : 'var(--surface)'};cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s;flex:0 0 auto;${monitoringCategory===key?'box-shadow:0 4px 16px rgba(0,0,0,0.08);':''}">
      ...
      <div style="text-align:left;">
        ...
        <div style="font-size:11px;color:${monitoringCategory===key ? c.color : 'var(--muted)'};opacity:.8;margin-top:1px;">${key==='ban' ? filteredVehiclesCount+' unit terdaftar' : filteredPelumasCount+' catatan'}</div>
      </div>
  ...
  <div style="display:flex;gap:0;margin-bottom:24px;background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:5px;width:fit-content;">
    <button onclick="monitoringTab='monitoring';render()" style="display:inline-flex;align-items:center;gap:8px;padding:10px 22px;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;border:1px solid transparent;font-family:'DM Sans',sans-serif;transition:all .2s;${isMonTab ? 'background:var(--green);color:white;box-shadow:0 4px 12px rgba(5,150,105,0.3);' : 'background:transparent;color:var(--muted);'}">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
      Monitoring
      <span style="background:${isMonTab?'rgba(255,255,255,0.25)':'#f3f4f6'};color:${isMonTab?'white':'var(--muted)'};font-size:11px;padding:1px 7px;border-radius:20px;font-weight:700;">${filterByUser(vehicles).length}</span>
    </button>
    <button onclick="monitoringTab='trial';render()" style="display:inline-flex;align-items:center;gap:8px;padding:10px 22px;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;border:1px solid transparent;font-family:'DM Sans',sans-serif;transition:all .2s;${!isMonTab ? 'background:#2563eb;color:white;box-shadow:0 4px 12px rgba(37,99,235,0.3);' : 'background:transparent;color:var(--muted);'}">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 3H5a2 2 0 0 0-2 2v4m6-6h10a2 2 0 0 1 2 2v4M9 3v18m0 0h10a2 2 0 0 0 2-2V9M9 21H5a2 2 0 0 1-2-2V9m0 0h18"/></svg>
      Trial
      <span style="background:${!isMonTab?'rgba(255,255,255,0.25)':'#f3f4f6'};color:${!isMonTab?'white':'var(--muted)'};font-size:11px;padding:1px 7px;border-radius:20px;font-weight:700;">${filterByUser(trialVehicles).length}</span>
    </button>
  </div>` : '';
  ...
  // Filter by current user - only show pelumas created by this user
  const filterPelumasByUser = (pelumasList) => {
    if (!currentUser) return [];
    // Administrator & Supervisor can see all data
    if (currentUser.role === 'administrator' || currentUser.role === 'supervisor') {
      return pelumasList;
    ...
    // Other users only see their own data
    return pelumasList.filter(r => r.createdBy === currentUser.id);
  };
  
  const activeRecs = filterPelumasByUser(isPelumasMon ? pelumasRecords : pelumasTrialRecords);
  ...
  <div style="display:flex;gap:0;margin-bottom:24px;background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:5px;width:fit-content;">
    <button onclick="pelumasTab='monitoring';render()" style="display:inline-flex;align-items:center;gap:8px;padding:10px 22px;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;border:1px solid transparent;font-family:'DM Sans',sans-serif;transition:all .2s;${isPelumasMon ? 'background:#7c3aed;color:white;box-shadow:0 4px 12px rgba(124,58,237,0.3);' : 'background:transparent;color:var(--muted);'}">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
      Monitoring
      <span style="background:${isPelumasMon?'rgba(255,255,255,0.25)':'#f3f4f6'};color:${isPelumasMon?'white':'var(--muted)'};font-size:11px;padding:1px 7px;border-radius:20px;font-weight:700;">${filterPelumasByUser(pelumasRecords).length}</span>
    </button>
    <button onclick="pelumasTab='trial';render()" style="display:inline-flex;align-items:center;gap:8px;padding:10px 22px;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;border:1px solid transparent;font-family:'DM Sans',sans-serif;transition:all .2s;${!isPelumasMon ? 'background:#7c3aed;color:white;box-shadow:0 4px 12px rgba(124,58,237,0.3);' : 'background:transparent;color:var(--muted);'}">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 3H5a2 2 0 0 0-2 2v4m6-6h10a2 2 0 0 1 2 2v4M9 3v18m0 0h10a2 2 0 0 0 2-2V9M9 21H5a2 2 0 0 1-2-2V9m0 0h18"/></svg>
      Trial Pelumas
      <span style="background:${!isPelumasMon?'rgba(255,255,255,0.25)':'#f3f4f6'};color:${!isPelumasMon?'white':'var(--muted)'};font-size:11px;padding:1px 7px;border-radius:20px;font-weight:700;">${filterPelumasByUser(pelumasTrialRecords).length}</span>
    </button>
  </div>`;
...
function renderAlerts() {
  ...
  // Filter vehicles by current user (except administrator)
  const getUserVehicles = () => {
    const allV = [...vehicles, ...trialVehicles];
    if (!currentUser) return [];
    if (currentUser.role === 'administrator') return allV;
    return allV.filter(v => v.createdBy === currentUser.id);
  };
  ...
  // Kendaraan overdue update >45 hari (gabungan monitoring + trial, filtered by user)
  const overdueAlerts = userVehicles.map(v => {
    const lastCheck = getLastCheckDate(v);
    if (lastCheck) {
...
function renderAlertsTechSupport() {
  // Notifikasi hilang HANYA jika klaim sudah Approved oleh admin/supervisor
  // Jika Rejected → tetap tampil agar TS bisa update ulang foto
  const myNotifs = TECH_NOTIFICATIONS.filter(n => {
    // Hanya tampilkan notif yang diambil oleh user ini atau belum diambil siapapun
    if (n.takenBy && n.takenBy !== currentUser.id) return false;
    const claim = CLAIMS.find(c => c.id === n.claimId);
    if (!claim) return true;
    // Hilang jika sudah Approved
  ...
  const notifCards = myNotifs.length === 0
    ? `<div class="empty-state" style="padding:60px 20px;">
        ...
        <p style="margin-top:14px;font-weight:600;">Semua klaim telah selesai.</p>
    ...
    : myNotifs.map(n => {
        ...
        const isRejected = claim && claim.status === 'Rejected';

        // Warna border card: merah jika rejected, hijau jika milik saya, biru jika baru
        const borderColor = isRejected ? '#fecdd3' : isMine ? '#a7f3d0' : '#bfdbfe';
        const iconBg      = isRejected ? 'var(--rose-light)' : isMine ? 'var(--green-light)' : 'var(--blue-light)';
        const iconColor   = isRejected ? 'var(--rose)' : isMine ? 'var(--green)' : 'var(--blue)';

        return `
        <div style="background:var(--surface);border:1.5px solid ${borderColor};border-radius:16px;padding:18px 20px;margin-bottom:12px;transition:box-shadow .15s,transform .15s;" onmouseover="this.style.boxShadow='0 4px 20px rgba(0,0,0,0.08)'" onmouseout="this.style.boxShadow='none'">

          ${isRejected ? `
          <div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:var(--rose-light);border:1px solid #fecdd3;border-radius:10px;margin-bottom:14px;">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#e11d48" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
            <div style="flex:1;">
              <div style="font-size:12px;font-weight:800;color:#9f1239;">❌ Klaim Ditolak oleh ${claim.resolvedBy || 'Administrator/Supervisor'}</div>
              ${claim.resolvedNote ? `<div style="font-size:11px;color:#9f1239;margin-top:2px;opacity:.85;">Alasan: ${claim.resolvedNote}</div>` : ''}
            </div>
            <span style="font-size:10px;font-weight:700;background:#fecdd3;color:#9f1239;border:1px solid #fca5a5;border-radius:20px;padding:2px 9px;white-space:nowrap;">Upload Ulang</span>
          </div>` : ''}
          ...
          <div style="display:flex;align-items:flex-start;gap:14px;">
            <div style="width:42px;height:42px;border-radius:12px;background:${iconBg};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
              ${isRejected
                ? `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="${iconColor}" stroke-width="2"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>`
                : `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="${iconColor}" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>`}
            </div>
            <div style="flex:1;min-width:0;">
              ...
              <div style="font-size:11px;color:var(--muted);display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                ...
                ${photoCount > 0 ? `<span style="color:${isRejected ? '#e11d48' : 'var(--green)'};font-weight:700;">📷 ${photoCount}/20 foto${isRejected ? ' (perlu diperbarui)' : ''}</span>` : ''}
              </div>
              ${isTaken ? `
              <div style="margin-top:10px;padding:8px 12px;border-radius:10px;background:${isMine ? (isRejected ? 'var(--rose-light)' : 'var(--green-light)') : '#f3f4f6'};border:1px solid ${isMine ? (isRejected ? '#fecdd3' : '#a7f3d0') : '#e5e7eb'};font-size:12px;font-weight:700;color:${isMine ? (isRejected ? '#9f1239' : '#065f46') : 'var(--muted)'};display:flex;align-items:center;gap:6px;">
            ...
            <div style="flex-shrink:0;display:flex;flex-direction:column;gap:8px;align-items:flex-end;">
              ${!isTaken
                ? `<button onclick="takeClaim('${n.claimId}')"
                    style="display:inline-flex;align-items:center;gap:7px;padding:10px 18px;background:linear-gradient(135deg,#2563eb,#1d4ed8);color:white;border:none;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;box-shadow:0 3px 10px rgba(37,99,235,0.3);transition:all .15s;white-space:nowrap;"
                    onmouseover="this.style.transform='translateY(-1px)';this.style.boxShadow='0 6px 16px rgba(37,99,235,0.4)'"
                    onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='0 3px 10px rgba(37,99,235,0.3)'">
                ...
                : isMine
                  ? `<button onclick="openProcessClaimModal('${n.claimId}')"
                      style="display:inline-flex;align-items:center;gap:7px;padding:10px 18px;background:${isRejected ? 'linear-gradient(135deg,#e11d48,#be123c)' : 'linear-gradient(135deg,#059669,#047857)'};color:white;border:none;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;box-shadow:0 3px 10px ${isRejected ? 'rgba(225,29,72,0.3)' : 'rgba(5,150,105,0.3)'};transition:all .15s;white-space:nowrap;"
                      ...
                      ${isRejected ? 'Upload Ulang Foto' : (photoCount > 0 ? 'Update Foto ('+photoCount+'/20)' : 'Upload Foto')}
                    </button>`
  ...
  const rejectedCount = myNotifs.filter(n => { const c = CLAIMS.find(x => x.id === n.claimId); return c && c.status === 'Rejected'; }).length;
  ...
  <div>
    <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        ...
        ${rejectedCount > 0 ? `<div style="padding:8px 14px;background:var(--rose-light);border:1px solid #fecdd3;border-radius:12px;font-size:12px;font-weight:700;color:#9f1239;display:flex;align-items:center;gap:6px;">
          ...
          ${rejectedCount} klaim ditolak — perlu diperbarui
        </div>` : ''}
      </div>
    </div>
    <div style="padding:10px 14px;background:var(--blue-light);border:1px solid #bfdbfe;border-radius:12px;font-size:12px;color:#1e40af;font-weight:600;margin-bottom:20px;display:flex;align-items:center;gap:8px;">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
      Ambil klaim → Upload foto proses (max 20) → Simpan → Menunggu keputusan Admin/Supervisor. Jika ditolak, upload ulang foto yang sesuai.
    </div>
    ${notifCards}
  </div>`;
}
...
function renderClaims() {
  ...
  function rowProses(c) {
    ...
    const hasPhotos = c.processPhotos && c.processPhotos.length > 0;
    return `
    <div class="claim-row" style="align-items:flex-start;${hasPhotos ? 'cursor:pointer;transition:box-shadow .15s,transform .15s;' : ''}"
      ${hasPhotos ? `onclick="openClaimPhotoViewer('${c.id}')" title="Klik untuk melihat ${c.processPhotos.length} foto proses"
        onmouseover="this.style.boxShadow='0 6px 24px rgba(37,99,235,0.13)';this.style.transform='translateY(-2px)'"
        onmouseout="this.style.boxShadow='';this.style.transform=''"` : ''}>
      <div style="width:42px;height:42px;background:${hasPhotos ? 'var(--blue-light)' : '#f3f4f6'};border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px;">
        ${hasPhotos
          ? `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--blue)" stroke-width="2"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>`
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:11px;color:var(--muted);margin-top:4px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
          <span style="display:flex;align-items:center;gap:4px;">
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            Diajukan: ${c.date}
          </span>
          ${c.takenByName ? `<span style="display:flex;align-items:center;gap:4px;font-weight:600;color:#1d4ed8;">
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            ${c.takenByName}
          </span>` : ''}
          ${hasPhotos ? `<span style="display:flex;align-items:center;gap:4px;font-weight:700;color:var(--blue);background:var(--blue-light);padding:2px 8px;border-radius:20px;border:1px solid #bfdbfe;">
            ...
            ${c.processPhotos.length} foto · klik untuk lihat
          </span>` : ''}
        </div>
      </div>
      <div style="flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:6px;">
        ...
        ${/* Tombol Update untuk Technical Support yang mengambil klaim ini */
          currentUser.role === 'technical_support' && c.status === 'In Progress' && c.takenBy === currentUser.id
            ? `<button onclick="event.stopPropagation();openProcessClaimModal('${c.id}')"
                style="display:inline-flex;align-items:center;gap:6px;padding:7px 13px;background:linear-gradient(135deg,#2563eb,#1d4ed8);color:white;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;box-shadow:0 3px 10px rgba(37,99,235,0.3);transition:all .15s;white-space:nowrap;"
                onmouseover="this.style.transform='translateY(-1px)';this.style.boxShadow='0 5px 14px rgba(37,99,235,0.4)'"
                onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='0 3px 10px rgba(37,99,235,0.3)'">
                ...
                Update${c.processPhotos && c.processPhotos.length > 0 ? ' ('+c.processPhotos.length+'/20)' : ''}
              </button>`
            : ''
        }
        ...
        ${/* Tombol lihat foto untuk admin/supervisor; atau TS melihat foto klaim orang lain */
          c.processPhotos && c.processPhotos.length > 0 && !(currentUser.role === 'technical_support' && c.takenBy === currentUser.id)
            ? `<button onclick="event.stopPropagation();openClaimPhotoViewer('${c.id}')"
                style="font-size:11px;font-weight:700;color:var(--blue);background:var(--blue-light);border:1px solid #bfdbfe;border-radius:8px;padding:4px 10px;cursor:pointer;display:flex;align-items:center;gap:4px;font-family:'DM Sans',sans-serif;margin-top:2px;">
        ...
        ${canEdit && !c.processDone && currentUser.role !== 'technical_support' ? `<span style="font-size:10px;color:var(--muted);">Diubah oleh ${currentUser.name}</span>` : ''}
      </div>
    </div>`;
  }
  ...
  function rowHasil(c) {
    ...
    const hasPhotos = c.processPhotos && c.processPhotos.length > 0;
    return `
    <div class="claim-row" style="border-color:${borderC};${hasPhotos ? 'cursor:pointer;transition:box-shadow .15s,transform .15s;' : ''}"
      ${hasPhotos ? `onclick="openClaimPhotoViewer('${c.id}')" title="Klik untuk melihat ${c.processPhotos.length} foto proses"
        onmouseover="this.style.boxShadow='0 6px 24px rgba(0,0,0,0.1)';this.style.transform='translateY(-2px)'"
        onmouseout="this.style.boxShadow='';this.style.transform=''"` : ''}>
...
function confirmClaimStatusChange() {
  ...
  // ── Notifikasi ke Sales jika ada submittedBy ──
  ...
  // ── Notifikasi ke Technical Support yang mengerjakan klaim ini ──
  if (isFinal && _uc && _uc.takenBy) {
    // Update TECH_NOTIFICATIONS: tandai status keputusan agar TS tahu
    TECH_NOTIFICATIONS = TECH_NOTIFICATIONS.map(n => {
      if (n.claimId !== _resolveClaimId) return n;
      const updated = {
        ...n,
        decisionStatus: _resolveNewStatus,  // 'Approved' | 'Rejected'
        decisionNote: note,
        decisionBy: currentUser.name,
        decisionAt: new Date().toISOString(),
        // Jika Approved → tandai read agar badge turun; jika Rejected → biarkan unread
        read: _resolveNewStatus === 'Approved' ? true : false,
      };
      supaUpsertTechNotif(updated);
      return updated;
    });

    // Update badge TS (jika sedang login)
    if (typeof updateAlertBadge === 'function') updateAlertBadge();

    // Toast popup realtime untuk TS yang sedang online pada sesi yang sama
    if (currentUser && currentUser.id === _uc.takenBy) {
      // Jarang terjadi (admin & TS biasanya beda sesi), tapi jaga-jaga
    }
  }

  closeResolveModal();
  if (_resolveNewStatus === 'Approved') { claimActiveTab = 'hasil'; claimResultQuery = ''; }
  render();
}

// ====== BUAT KLAIM BARU (ADMIN / SUPERVISOR) ======
let TECH_NOTIFICATIONS = [];  // notifikasi untuk technical support
...
function openBuatKlaimModal() {
  if (currentUser.role !== 'administrator' && currentUser.role !== 'supervisor') return;
  // Reset form
...
function openProcessClaimModal(claimId) {
  ...
  // Load existing photos — TS bisa tambah foto kapan saja selama klaim masih In Progress
  _processClaimPhotos = claim.processPhotos ? [...claim.processPhotos] : [];

  // Update judul modal sesuai konteks
  const titleEl = document.querySelector('#modal-process-claim .modal-title');
  if (titleEl) {
    titleEl.textContent = _processClaimPhotos.length > 0 ? 'Update Foto Klaim Ban' : 'Proses Klaim Ban';
  }
  // Update teks tombol simpan
  const saveBtn = document.getElementById('pc-save-btn');
  if (saveBtn) {
    saveBtn.innerHTML = _processClaimPhotos.length > 0
      ? `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v14a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg> Simpan Update Foto`
      : `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v14a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg> Simpan & Tandai Selesai`;
  }
  // Isi catatan yang sudah ada sebelumnya
  const notesEl = document.getElementById('pc-notes');
  if (notesEl && claim.processNotes) notesEl.value = claim.processNotes;
...
function submitProcessClaim() {
  ...
  const photoCount = _processClaimPhotos.length;
  ...
  toast.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg> ${photoCount} foto tersimpan — menunggu keputusan Administrator/Supervisor`;
  document.body.appendChild(toast);
...
function submitAddVehicle(e) {
  ...
  const v = {
    ...
    createdBy: currentUser ? currentUser.id : '',  // Track creator
