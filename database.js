...
//   CREATE POLICY "allow_all" ON user_notifications FOR ALL USING (true);
...
const DEFAULT_USERS = [
  ...
  // Akun bypass untuk testing - HAPUS DI PRODUCTION!
  { id: 'test1', name: 'Admin Test',      email: 'admin@test.com',      password: '1', role: 'administrator',     color: '#dc2626', initial: 'A' },
  { id: 'test2', name: 'Supervisor Test', email: 'supervisor@test.com', password: '1', role: 'supervisor',        color: '#ea580c', initial: 'S' },
  { id: 'test3', name: 'Tech Test',       email: 'tech@test.com',       password: '1', role: 'technical_support', color: '#16a34a', initial: 'T' },
  { id: 'test4', name: 'Sales Test',      email: 'sales@test.com',      password: '1', role: 'sales',             color: '#2563eb', initial: 'S' },
  { id: 'test5', name: 'Counter Test',    email: 'counter@test.com',    password: '1', role: 'sales_counter',     color: '#7c3aed', initial: 'C' },
  { id: 'test6', name: 'Viewer Test',     email: 'viewer@test.com',     password: '1', role: 'viewer',            color: '#64748b', initial: 'V' },
];
...
// ── Backward-compat wrappers (tetap ada agar kode lama tidak error) ──────────
...
// ============================================================
// SISTEM NOTIFIKASI UNIVERSAL — user_notifications
// Setiap notifikasi disimpan per-userId di Supabase sehingga
// user yang OFFLINE tetap menerima notifikasi saat login kembali.
// ============================================================

// Simpan 1 notif untuk 1 user spesifik
...
// Kirim notif ke semua user dengan role tertentu (ambil dari DB)
async function pushNotifToRole(role, notif) {
  try {
    const { data: users } = await supa.from('users').select('id').eq('role', role);
    if (!users) return;
    for (const u of users) {
      await pushNotifToUser(u.id, notif);
    }
  } catch(e) { console.warn('pushNotifToRole error', role, e); }
}

// Kirim ke semua technical_support
async function pushNotifToAllTechSupport(notif) {
  await pushNotifToRole('technical_support', notif);
}

// Kirim ke semua administrator + supervisor
...
// Load semua notifikasi milik user yang sedang login dari Supabase
async function loadUserNotifications() {
  if (!currentUser) return;
  try {
    const { data } = await supa.from('user_notifications').select('*');
    if (data) {
...
async function markUserNotifRead(notifId) {
  ...
  if (updated) {
    try { await supa.from('user_notifications').upsert({ id: notifId, data: updated }); } catch(e) {}
  }
...
async function markAllUserNotifsRead() {
  ...
  USER_NOTIFICATIONS = USER_NOTIFICATIONS.map(n =>
...
async function loadFromSupabase() {
  try {
    const [v, tv, al, cl, pr, ptr, dt, ch, bor, botr, ar, atr] = await Promise.all([
      ...
      supaLoadAll('ban_otr_records'),
      supaLoadAll('ban_otr_trial_records'),
      supaLoadAll('aki_records'),
      supaLoadAll('aki_trial_records'),
    ]);
    ...
    await loadUserNotifications();  // ← universal notif per-user
    await loadRolePerms();
    return true;
...
function applySession(user) {
  ...
  // Export: gunakan rolePerms jika tersedia, fallback ke admin check
...
function _showToastNotif(bg, html, onClick) {
  ...
  t.style.cssText = 'position:fixed;top:60px;right:20px;background:' + bg + ';color:white;padding:14px 20px;border-radius:16px;font-size:13px;font-weight:700;z-index:9999;box-shadow:0 8px 30px rgba(0,0,0,0.3);max-width:340px;cursor:pointer;transition:opacity .3s;line-height:1.5;';
  t.innerHTML = html;
...
function startRealtimeSubscription() {
  ...
  // ── user_notifications — UNIVERSAL, semua role, persisten ───────────────────
  channel.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'user_notifications' }, (payload) => {
    ...
    // Toast popup sesuai tipe
    const type = notif.type || '';
    let bg = '#2563eb', icon = '🔔';
    if (type === 'claim_task')                                { bg = 'linear-gradient(135deg,#0e7490,#0891b2)'; icon = '🔔'; }
    else if (type === 'claim' && notif.status === 'Approved') { bg = 'linear-gradient(135deg,#059669,#047857)'; icon = '✅'; }
    else if (type === 'claim' && notif.status === 'Rejected') { bg = 'linear-gradient(135deg,#e11d48,#be123c)'; icon = '❌'; }
    else if (type === 'duty'  && notif.status === 'Approved') { bg = 'linear-gradient(135deg,#059669,#047857)'; icon = '✅'; }
    else if (type === 'duty'  && notif.status === 'Rejected') { bg = 'linear-gradient(135deg,#e11d48,#be123c)'; icon = '❌'; }
    else if (type === 'monitoring' || type === 'trial')       { bg = 'linear-gradient(135deg,#d97706,#b45309)'; icon = '📋'; }
    _showToastNotif(bg,
      icon + ' <strong>' + notif.title + '</strong><br><span style="font-weight:500;font-size:12px;opacity:.9;">' + notif.desc + (notif.note ? '<br>' + notif.note : '') + '<br>Klik untuk lihat</span>',
      () => navigate('alerts'));
  });
  ...
  // ── Legacy: tech_notifications ──────────────────────────────────────────────
  channel.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'tech_notifications' }, (payload) => {
    const notif = payload.new?.data; if (!notif || !currentUser || currentUser.role !== 'technical_support') return;
  ...
  channel.on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'tech_notifications' }, (payload) => {
    const updated = payload.new?.data; if (!updated || !currentUser || currentUser.role !== 'technical_support') return;
