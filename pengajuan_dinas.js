...
function submitPengajuanDinas() {
  ...
  // ✅ Kirim notif persisten ke admin & supervisor (terima walau offline)
  if (typeof pushNotifToAdminSupervisor === 'function') {
    pushNotifToAdminSupervisor({
      type:   'duty',
      status: 'Pending',
      title:  '🚗 Pengajuan Dinas Luar Kota',
      desc:   name + ' → ' + dest + ' (' + fmt(start) + ' – ' + fmt(end) + ')',
      note:   purpose + (document.getElementById('pd-company').value.trim() ? ' — ' + document.getElementById('pd-company').value.trim() : ''),
    });
  }
  // ✅ Konfirmasi persisten ke user yang submit sendiri
  if (currentUser && typeof pushNotifToUser === 'function') {
    pushNotifToUser(currentUser.id, {
      type:   'duty',
      status: 'Pending',
      title:  '✅ Pengajuan Dinas Terkirim',
      desc:   'Tujuan: ' + dest + ' (' + fmt(start) + ' – ' + fmt(end) + ')',
      note:   'Menunggu persetujuan Administrator / Supervisor.',
    });
  }
